...
class LearningEngine:
    ...
    def _merge_defaults(weights: Dict[str, Any]) -> Dict[str, Any]:
        ...
        import copy
        merged = copy.deepcopy(LearningEngine.DEFAULT_WEIGHTS)
        ...
        if "confidence_calibration" in weights:
            ...
            weights = {k: v for k, v in weights.items() if k != "confidence_calibration"}
    ...
    def save_all_weights(all_weights: Dict[str, Any]):
        ...
        os.makedirs(LEARNING_DB.parent, exist_ok=True)
    ...
    @staticmethod
    def update_weights(engine_id: str = None) -> Dict[str, Any]:
        """
            ...
            if league in rule_perf:
                for rule, stats in rule_perf[league].items():
                    if stats["total"] >= 5:  # Lower threshold for faster adaptation
                        ...
                        # Dynamic Adjustment Factor (0.1 step)
                        if accuracy > 0.65:
        ...
        LearningEngine.sync_to_supabase(all_weights)
        return all_weights

    @staticmethod
    def sync_to_supabase(all_weights: Dict[str, Any]):
        """Push learning weights to Supabase for cloud persistence and UI access."""
        try:
            ...
            for league, w in all_weights.items():
                ...
                rows.append({
                    ...
                    'predictions_analyzed': sum(
                        1 for _ in []
                    ),  # Will be populated by analyze_performance
                })
                w['confidence_calibration'] = conf_cal  # Restore for local use

            if rows:
                # Use existing event loop if available, otherwise create one
                try:
                    loop = asyncio.get_running_loop()
                    loop.create_task(sync.batch_upsert('learning_weights', rows))
                except RuntimeError:
                    asyncio.run(sync.batch_upsert('learning_weights', rows))
                print(f"   [Cloud] Synced {len(rows)} learning weight entries to Supabase.")
        except Exception as e:
            print(f"   [Cloud] Learning weights sync skipped: {e}")
