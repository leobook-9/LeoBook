...
def load_model(checkpoint_path=None):
    ...
    models_dir = os.path.join(PROJECT_ROOT, "Data", "Store", "models")
    default_path = os.path.join(models_dir, "leobook_base.pth")

    # ── Path resolution ───────────────────────────────────────────────────────
    # Priority:
    #   1. Explicitly passed --checkpoint path
    #   2. leobook_base.pth  (written by trainer.save() at end of full run)
    #   3. phase1_latest.pth (written after every training day — survives Ctrl+C)
    #   4. Most recent phase*_day*.pth checkpoint in checkpoints/
    ...
    if path is None:
        if os.path.exists(default_path):
            ...
            # Fall back to latest checkpoint saved during training
            latest_path = os.path.join(models_dir, "phase1_latest.pth")
            if os.path.exists(latest_path):
                path = latest_path
                print(f"  ℹ  leobook_base.pth not found — using phase1_latest.pth")
                print(f"     (Training was interrupted before completion.)")
                print(f"     To use a specific checkpoint: --checkpoint <path>")
            else:
                # Try any checkpoint in checkpoints/
                ...
                if os.path.isdir(ckpt_dir):
                    ...
                    if candidates:
                        ...
                        print(f"  ℹ  leobook_base.pth not found — using {os.path.basename(path)}")
                        print(f"     (Training was interrupted before completion.)")

    if path is None or not os.path.exists(path):
        print(f"  ✗ No model checkpoint found.")
        print(f"    Expected: {default_path}")
        print(f"    Run: python Leo.py --train-rl   to train the model first.")
        sys.exit(1)
